  ...
  <div class="items">
    <div class="container">
      <div class="search-container">
        <form @submit.prevent="getItems">
          <input
            ...
            id="searchquery"
            v-model="querytext"
            placeholder="search based on name, type, attributes.."
          />
          <button type="submit">Search</button>
          <input type="button" value="Create Item" @click="load('/newitem')" />
          <div class="edit">
            <input type="checkbox" v-model="editable" @change="getItems()" />
            Edit my entries
          </div>
        </form>
      </div>
      ...
      <div class="item-container">
        <div
          ...
          :class="{ extended: item.show }"
          :key="item._id"
          @click="show(item)"
        >
          <div class="card-org">
            <div class="icon" :style="{ backgroundColor: item.color }">
              <img :src="require(`../assets/icons/items/${item.icon}`)" />
            </div>
            <div class="item-button">
              <div @click="load(`/edititem/${item._id}`)" v-if="editable">
                edit
              </div>
              <div @click.stop="addtoCharacter(id)" v-if="!editable">
                Add to Character
              </div>
            </div>
            <div class="item-text">
          ...
          <div class="item-content" v-if="item.show">
            ...
            <div class="item-extra">
              <div
                v-for="[key, value] in Object.entries(item.display())"
                :key="key"
              >
                <b>{{ key }}:</b><i>{{ value }}</i>
              </div>
    ...
    <div v-if="addshow" class="popup">
      ...
      <select class="input wide" v-model="selected">
        <option :value="id" v-for="(character, id) in characters" :key="id">
...
export default {
  ...
  methods: {
    async getItems() {
      this.items = [];
      var temp = await itemService.getItems(
        this.querytext,
        this.curpage,
        this.editable
      );
    ...
    async save() {
      var char = await characterService.getCharacter(
        this.characters[this.selected]._id
      );
