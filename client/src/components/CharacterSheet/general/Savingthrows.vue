<template>
  <div class="savingthrows">
    ...
    <div
      class="save clickable"
      @click="open(saveid)"
      v-for="(savingthrow, saveid) in savingthrows"
      :key="saveid"
    >
      <h5>{{ saveid }}</h5>
      <div class="save-mod">
        <h2>
          <b
            v-if="
              savingthrow.bonus(
                character.attributes[saveid].mod,
      ...
      <h6>
        <i v-if="savingthrow.proficiency == 1">Proficient</i
        ><i v-else-if="savingthrow.proficiency == 2">Expert</i>
      </h6>
    </div>
    <popup v-show="atpop" @close="close">
      <div class="popup long">
        ...
        <h3>
          {{
            character.savingthrows[updateid].bonus(
              character.attributes[updateid].mod,
              character.proficiency
            )
          }}
        </h3>
        <h5>Proficiency:</h5>
        Proficient:<input
          v-model="character.savingthrows[updateid].proficiency"
          type="checkbox"
        />
        <h5>Modifiers:</h5>
        <div class="scrollcontainer">
          <div
            class="mod-div"
            v-for="(save, id) in character.savingthrows[updateid].modifiers"
            :key="save._id"
          >
            <h5>Name:</h5>
            <input class="input wide" type="text" v-model="save.name" />
            <h5>Value:</h5>
            <input
              class="input small"
              type="number"
            ...
            /><br />
            <h5>Source:</h5>
            <input class="input wide" type="text" v-model="save.source" /><br />
            <button @click="removemodifier(updateid, id)">
              Remove Modifier
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    addmodifier(updateid) {
      ...
      temp.savingthrows[updateid].modifiers[
        this.savingthrows[updateid].modifiers.length
      ] = { name: "", value: 0, source: "" };
      this.$emit("update", [{ task: "update", data: temp }]);
    },
