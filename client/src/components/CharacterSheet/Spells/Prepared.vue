<template>
  <div class="prepared">
    ...
    <div class="scroll" @dragover="allowDrop($event)" @drop="drop($event)">
      <div
        class="clickable spell"
        @click="showSpell(id)"
        v-for="(spell, id) in preparedSpells"
        :key="id"
        draggable="true"
      ...
      >
        <div class="spell-upper">
          <div class="spellname">
            ...
            <img
              ...
              class="concentration"
              src="../../../assets/icons/concentration.svg"
            />
          </div>
          <div class="spelllevel">
            <h5>
              {{ spell.level }}<i v-if="spell.level == parseInt(1)">st</i>
              <i
                v-else-if="
                  (spell.level > 3 && spell.level < 21) || spell.level == 0
                "
                >th</i
              >
              <i v-else-if="spell.level == 2">nd</i>
              <i v-else>rd</i>
              level {{ spell.school }}<i> ({{ spell.components }})</i>
            </h5>
          </div>
          <div class="action">
            <h5>Casting:{{ spell.castingtime }}</h5>
          </div>
          <div class="distance">
            <h5>Distance: {{ spell.distance }}</h5>
          </div>
          <div class="attack">
            <h5>Attack/Save: {{ spell.attack }}</h5>
          </div>
        </div>
        <div class="extra-spell" v-if="showid == id">
          <div class="extra-upper">
            ...
            <div>
              <h5>Effect/Damage:{{ spell.effect }}</h5>
            </div>
          </div>
          <div class="spell-description">{{ spell.description }}</div>
          <div class="spell-buttons">
            <button @click.stop="unprepare(id)">Un-Prepare</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    unprepare(id) {
      const index = this.spells.findIndex((o) =>
        o.equals(this.preparedSpells[id])
      );
